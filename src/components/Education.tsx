        ...
        <section className="my-8 px-4 max-w-3xl mx-auto">
                ...
                {education.map((edu, index) => (
                    <li key={index} className="flex flex-col md:flex-row items-center md:items-start space-y-2 md:space-y-0 md:space-x-4">
                        <div className="w-16 h-16 bg-gray-200 rounded-full overflow-hidden flex-shrink-0">
                            <img src={edu.logo} alt={t('education.logoAlt', { institution: t(edu.institution) })} className="w-full h-full object-cover" loading="lazy" />
                        </div>
            ...
            <ul className="mt-6 space-y-6">
                {certificates.map((cert, index) => (
                    <li key={index} className="flex flex-col md:flex-row items-center md:items-start space-y-2 md:space-y-0 md:space-x-4">
                        <div className="w-16 h-16 bg-gray-200 rounded-full overflow-hidden flex-shrink-0">
                            <img src={cert.logo} alt={t('education.certLogoAlt', { title: t(cert.title) })} className="w-full h-full object-cover" loading="lazy" />
                        </div>
                        <div className="text-center md:text-left">
                            <h3 className="text-cyan-200 text-lg sm:text-xl font-semibold">
                                <a href={t(cert.link)} target="_blank" rel="noopener noreferrer" className="hover:underline hover:text-cyan-300" aria-label={t('education.viewCertificate', { title: t(cert.title) })}>
                                    {t(cert.title)}
                                </a>
                            </h3>
